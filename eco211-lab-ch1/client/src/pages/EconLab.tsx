...
function ResultsScreen({ score, results, onRestart, courseTitle }: {
  ...
  const stationRows211 = Object.entries({
  ...
  }).filter(([id]) => sectionScores[id]).map(([id, label]) => ({ label, ...sectionScores[id] }));

  function printPDF() {
    const w = window.open("", "_blank", "width=820,height=960");
    if (!w) return;
    const now = new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });
    const stationTableRows = stationRows211
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>ECO 211 Ch1 Results</title>
      <style>
      ...
      <footer>Content adapted from OpenStax. Access for free at https://openstax.org/books/principles-microeconomics-3e/pages/1-introduction</footer>
      </body></html>`);
...
// Main EconLab
// ─────────────────────────────────────────────
const STORAGE_KEY = "econlab211_done_ch1";
